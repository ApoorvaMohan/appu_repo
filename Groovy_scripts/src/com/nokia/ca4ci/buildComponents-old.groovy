			...
			source ${WORKSPACE}/devops_ci/scripts/buildscripts/env_cp.sh $profile
		        ...
		        do
			          ...
			          if [ "${profile}" != "Cloudera57" ]; then
			                mvn versions:set -U -B -DnewVersion=${rpm_version} -DnoClouderaRepo && mvn exec:exec -U -B -DapplyProfile -pl . -DnoClouderaRepo && mvn clean -f pom-${profile}.xml -DnoClouderaRepo && mvn deploy -fae -B -U -f pom-${profile}.xml -D${profile} -DnoClouderaRepo -Dmaven.wagon.http.ssl.insecure=true -DaltDeploymentRepository=cemod-mvn-snapshots::default::https://repo.lab.pl.alcatel-lucent.com/cemod-mvn-candidates/ || EXIT=$?
           			  else
			                mvn versions:set -U -B -DnewVersion=${rpm_version} -DnoWandiscoRepo && mvn exec:exec -U -B -DapplyProfile -pl . -DnoWandiscoRepo && mvn clean -f pom-${profile}.xml -DnoWandiscoRepo && mvn deploy -fae -U -B -f pom-${profile}.xml -D${profile} -Dmaven.wagon.http.ssl.insecure=true -DnoWandiscoRepo -DaltDeploymentRepository=cemod-mvn-snapshots::default::https://repo.lab.pl.alcatel-lucent.com/cemod-mvn-candidates/ || EXIT=$?
           			  fi
			          cd -	
