		...
		for(profile in  BUILD_PROFILE) {
			...
			for(folder in FOLDERS) {
				dir(folder) {
					...
					if( profile != 'Cloudera' ) {
						sh "export WORKSPACE=${WORKSPACE} && export profile=${profile} && mvn versions:set -U -DnewVersion=${rpm_version} -DnoClouderaRepo && mvn exec:exec -U -DapplyProfile -pl . -DnoClouderaRepo && mvn clean -f pom-${profile}.xml -DnoClouderaRepo && mvn deploy -fae -U -B -f pom-${profile}.xml -D${profile} -DnoClouderaRepo -Dmaven.wagon.http.ssl.insecure=true  -Dcemod-deploy-artifactory -Dartifactory.username=${OS_USERNAME} -Dartifactory.password=${OS_PASSWORD} -DskipTests=true"
                                        }
					else {
						sh "export WORKSPACE=${WORKSPACE} && export profile=${profile} && mvn versions:set -U -DnewVersion=${rpm_version} -DnoWandiscoRepo && mvn exec:exec -U -DapplyProfile -pl . -DnoWandiscoRepo && mvn clean -f pom-${profile}.xml -DnoWandiscoRepo && mvn deploy -fae -U -B -f pom-${profile}.xml -D${profile} -Dmaven.wagon.http.ssl.insecure=true -DnoWandiscoRepo -Dcemod-deploy-artifactory -Dartifactory.username=${OS_USERNAME} -Dartifactory.password=${OS_PASSWORD}  -DskipTests=true"
					}
				}
			}
...
def buildAnyComponent(folderPaths) {
		...
		for(folder in FOLDERS) {
					...
					sh "export WORKSPACE=${WORKSPACE} && mvn -U versions:set -DnewVersion=${rpm_version} && mvn clean deploy -B -ff -U -Dmaven.javadoc.skip=true -Dmaven.wagon.http.ssl.insecure=true -Dcemod-deploy-artifactory -Dartifactory.username=${OS_USERNAME} -Dartifactory.password=${OS_PASSWORD} -DskipTests=true"	
				}
		}
                if( env.publish == 'true' ) {
                	def lib = new com.nokia.ca4ci.publish()
...
def buildAnyComponentWithoutDeploy(folderPaths) {
                ...
                for(folder in FOLDERS) {
                                dir(folder) {
                                        ...
                                        sh "export WORKSPACE=${WORKSPACE} && mvn -U versions:set -DnewVersion=${rpm_version} && mvn clean install -B -ff -U -Dmaven.javadoc.skip=true -Dmaven.wagon.http.ssl.insecure=true -Dcemod-deploy-artifactory -Dartifactory.username=${OS_USERNAME} -Dartifactory.password=${OS_PASSWORD} -DskipTests=true"
                                }
                }
                if( env.publish == 'true' ) {
                        def lib = new com.nokia.ca4ci.publish()
